...
unsigned *construct_lattice(char *geometry, FILE *input, unsigned *ns, unsigned *nn, unsigned *ng, double **s){
	unsigned *nnt;
	if(!strcmp(geometry, "cubic")){
		nnt = construct_cubic(input, ns, nn, ng, s);
	// Geometries below are not tested!
...
unsigned *construct_cubic(FILE *input, unsigned *ns, unsigned *nn, unsigned *ng, double **s){
	unsigned l, dim;
	unsigned *N, *nnt;
	char boundaries[100];
	...
	*ns = N[d-1]*l;
	...
	const unsigned generators = *ng;

	nnt = malloc(2 * neighbours * size * sizeof(unsigned));
	*s = malloc(generators * links * size * sizeof(double));
	...
	for(unsigned i = 0; i < size; i++){
		for(unsigned d = 0; d < dim; d++){
			...
			// nearest sites
			shift = i*neighbours;
			nnt[shift + 2*d]     = generators * (i + ((id+1  )%l - id)*N[d]); // right, top,...
			nnt[shift + 2*d + 1] = generators * (i + ((id+l-1)%l - id)*N[d]); // left, bottom,...
			const unsigned im = nnt[shift + 2*d + 1];

			// nearest links
			shift = (size + i)*neighbours;
			nnt[shift + 2*d]     = generators * (d + links*i); // right, top,...
			nnt[shift + 2*d + 1] = generators * d + links*im; // left, bottom,...
		}
	}
